...
const checkoutSchema = z.object({
  ...
  cardExpiry: z.string().refine(
    (val) => /^(0[1-9]|1[0-2])\/([0-9]{2})$/.test(val), 
    "Expiry date must be in MM/YY format"
  ),
  cardCvc: z.string().refine(
    (val) => /^\d{3,4}$/.test(val), 
...
const Checkout = () => {
  ...
  // Sample cart data - in a real app, this would come from a context or store
  const cartItems = [
    {
      id: 1,
  ...
  const onSubmit = async (data: CheckoutFormValues) => {
    ...
    try {
      // In a real application, this is where you'd call a payment processing API
                ...
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                  ...
                  <Button type="submit" className="w-full" disabled={isProcessing}>
                    {isProcessing ? (
                      ...
                      <>
                        Place Order <ChevronRight className="ml-2" size={16} />
                      </>
                    )}
                  </Button>
                </form>
